...
export default function Home() {
  ...
  // ✅ 修复点 1：引入 alarmData 字典，支持多个大棚异常数据的加载
  useEffect(() => {
    setHasMounted(true);
    ...
    if (typeof window !== 'undefined') {
      ...
      if (targetId) {
        ...
        if (targetGhouse) {
          ...
          // 建立异常大棚的静态数据字典
          const alarmData: Record<string, {temp: number, hum: number}> = {
            '043': { temp: 36.5, hum: 88 },
            '089': { temp: 36.6, hum: 88 },
            '077': { temp: 32.5, hum: 75 },
            '092': { temp: 32.4, hum: 75 }
          };

          const alarm = alarmData[cleanId];
          const alarmTemp = params.get('temp');
          const alarmHum = params.get('hum');
          
          const initTemp = alarmTemp ? parseFloat(alarmTemp) : (alarm ? alarm.temp : targetGhouse.baseTemp);
          const initHum = alarmHum ? parseInt(alarmHum) : (alarm ? alarm.hum : targetGhouse.baseHum);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const alarm = alarmData[activeGhouse.id];
      const currentBaseTemp = alarm ? alarm.temp : activeGhouse.baseTemp;
      const currentBaseHum = alarm ? alarm.hum : activeGhouse.baseHum;
